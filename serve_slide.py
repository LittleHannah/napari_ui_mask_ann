...
流程：
  1. 把所有 tile 的 ST_RGB 组装成 slide 级别单个文件（首次运行保存，之后读缓存）
  2. 用全局 PCA + 坐标文件构建 slide-level embedding canvas（首次运行保存，之后读缓存）
...
from slide_viewer import _discover_tiles, assemble_st_rgb, build_global_emb_canvas
...
EMB_DOWNSAMPLE = 4   # embedding 降采样倍数（ds=4 → ~128MB canvas）


def prepare_slide_files(input_dir: str, mask_dir: str,
                        pca_path: str, coords_path: str,
                        downsample: int = 4, tile_size: int = 512) -> str:
    """
    保证以下文件存在（缓存命中则跳过）：
      pca_rgb_no/slide_rgb.png    ← assembled ST_RGB
      emb/slide_emb.npy           ← global PCA embedding canvas
    返回 prefix = "slide"。
    """
    prefix   = "slide"
...
if __name__ == "__main__":
    prefix = prepare_slide_files(
        INPUT_DIR, MASK_DIR, PCA_PATH, COORDS_PATH, EMB_DOWNSAMPLE
    )
